...
const projects: Project[] = [
  ...
  },
  {
    ...
    title: {
      en: "RAG Research Lab — Chunking & Embedding Strategies",
      fr: "Laboratoire de Recherche RAG — Stratégies de Chunking & Embeddings",
    },
    tagline: {
      en: "Open-source full-stack platform benchmarking state-of-the-art RAG chunking and embedding techniques",
      fr: "Plateforme full-stack open-source pour benchmarker les techniques de chunking et d'embeddings RAG état de l'art",
    },
    description: {
      en: "Built and published an open-source research platform to implement, compare, and benchmark multiple state-of-the-art chunking and embedding strategies for RAG pipelines — from fixed-size and semantic chunking to hierarchical parent-child, late chunking, contextual retrieval, and hybrid methods.",
      fr: "Développement et publication d'une plateforme de recherche open-source pour implémenter, comparer et benchmarker plusieurs stratégies de chunking et d'embeddings état de l'art pour les pipelines RAG — du chunking fixe et sémantique au hiérarchique parent-enfant, late chunking, retrieval contextuel et méthodes hybrides.",
    },
    details: {
      en: [
        "Implemented 10+ chunking strategies: fixed-size, sentence-aware, recursive/structural, Markdown header, academic section detection, semantic, hierarchical parent-child, late chunking, contextual retrieval, and hybrid pipelines",
        "Late chunking with Jina-Embeddings-v3 (sentence-transformers + PyTorch CPU) — embeds full document then splits token-level embeddings for context-aware chunks",
        "Semantic chunking via cosine similarity between consecutive sentences — topic-coherent boundaries without character-count limits",
        "Hierarchical parent-child retrieval — small child chunks indexed for precision, large parent chunks returned to the LLM for context",
        "Contextual retrieval (Anthropic 2024) — LLM-generated context prepended to each chunk before embedding (+49% retrieval improvement)",
        "Document parsing pipeline: PyMuPDF4LLM + Selectolax for structured text extraction from PDFs and web content",
        "FastAPI backend with configurable strategy parameters; React + Vite + TypeScript UI to upload documents and visualise chunking results side-by-side",
        "HuggingFace Hub integration with custom model downloader; fully containerised with Docker Compose",
      ],
      fr: [
        "Implémentation de 10+ stratégies de chunking : fixe, sentence-aware, récursif/structurel, Markdown header, détection de sections académiques, sémantique, hiérarchique parent-enfant, late chunking, retrieval contextuel et pipelines hybrides",
        "Late chunking avec Jina-Embeddings-v3 (sentence-transformers + PyTorch CPU) — embedding du document entier puis découpage au niveau token pour des chunks contextualisés",
        "Chunking sémantique via similarité cosinus entre phrases consécutives — frontières cohérentes par sujet sans limite de caractères",
        "Retrieval hiérarchique parent-enfant — petits chunks enfants indexés pour la précision, grands chunks parents retournés au LLM pour le contexte",
        "Retrieval contextuel (Anthropic 2024) — contexte généré par LLM préfixé à chaque chunk avant embedding (+49% d'amélioration du retrieval)",
        "Pipeline de parsing de documents : PyMuPDF4LLM + Selectolax pour l'extraction structurée de PDFs et contenus web",
        "Backend FastAPI avec paramètres de stratégie configurables ; UI React + Vite + TypeScript pour uploader des documents et visualiser les résultats côte à côte",
        "Intégration HuggingFace Hub avec service de téléchargement de modèles personnalisé ; entièrement conteneurisé avec Docker Compose",
      ],
    },
    stack: [
      "Python",
